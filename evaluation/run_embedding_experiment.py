...
REPORT_JSON = Path(__file__).with_name("embedding_experiment_dev.json")
REPORT_MARKDOWN = Path(__file__).with_name("embedding_experiment_dev.md")


def evaluate_embedding_experiment(provider, top_k: int = 5) -> dict:
    if top_k < 5:
    ...
    report["decision"] = {
        ...
        "status": (
            ...
            else "not_a_standalone_replacement"
        ),
    }
    return report
...
def markdown_report(report: dict) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "Document embeddings are cached by exact text for the duration of the run. Each dev "
            ...
            "performance, expanded SIEM evidence, and the RAG prototype case. Remaining partial "
            "misses include one MLOps training unit and one cross-functional leadership unit.",
            "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run paid embedding evaluation on dev")
    parser.add_argument("--top-k", type=int, default=5)
    parser.add_argument("--write-report", action="store_true")
    args = parser.parse_args()

    provider = OpenAIEmbeddingProvider(EmbeddingConfig.from_environment())
    report = evaluate_embedding_experiment(provider, top_k=args.top_k)
